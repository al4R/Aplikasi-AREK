...
class ResetPasswordActivity : AppCompatActivity() {
    ...
    private fun reset(){
        if(rp_password.length() < 7 ){
            rp_password.requestFocus()
            rp_password.error = "minimal 8 karakter"
            return
        } else if (rp_password.text!!.isEmpty()){
            rp_password.error = "Kata sandi tidak boleh kosong"
            rp_password.requestFocus()
            return
        }else if (rp_confirm.text!!.isEmpty()){
            rp_confirm.error = "Konfirmasi tidak boleh kosong"
            rp_confirm.requestFocus()
            return
        }else if(rp_password.text.toString() != rp_confirm.text.toString()){
            rp_password.requestFocus()
            rp_confirm.requestFocus()
            rp_confirm.error = "Periksa kata sandi anda"
            rp_password.error = "Periksa kata sandi anda"
            return
        }
        val user = s.getUser()
        val id = user!!.id
            ...
            override fun onResponse(call: Call<ResponModel>, response: Response<ResponModel>) {
                ...
                if (respon != null) {
                    if (respon.success == 1){
                        ...
                        Toast.makeText(this@ResetPasswordActivity,"Kata sandi direset",Toast.LENGTH_SHORT).show()
