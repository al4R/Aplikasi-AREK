...
class EditPasswordActivity : AppCompatActivity() {
    ...
    private fun editPass(){
        if(edit_password_lama.text!!.isEmpty()){
            ...
            return
        }else if(edit_konfirmasi.text!!.isEmpty()){
            edit_konfirmasi.error ="Tidak boleh kosong"
            edit_konfirmasi.requestFocus()
            return
        }
        ...
        else if(edit_password_lama.length() < 7){
            edit_password_lama.error = "Minimal 8 karakter"
            edit_password_lama.requestFocus()
            return
        }
