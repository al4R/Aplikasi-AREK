...
class LupaPasswordActivity : AppCompatActivity() {
    ...
    private fun kirim(){
       ...
       else if (lp_nik.length() < 15){
            lp_nik.requestFocus()
            lp_nik.error = " NIK tidak valid"
            return
        }
        else if(lp_email.text.toString().isNotEmpty()) {
            if (Patterns.EMAIL_ADDRESS.matcher(lp_email.text.toString()).matches() == false) {
                lp_email.error = "Email tidak valid "
                lp_email.requestFocus()
                return
            }
        }
    ...
    private fun hubungi(){
        try {
            ...
            if(i.resolveActivity(packageManager) != null){
                ...
                Toast.makeText(this, "Harap menginstall Whatsapp terlebih dahulu", Toast.LENGTH_SHORT).show()
            }
        }catch (e: Exception){
