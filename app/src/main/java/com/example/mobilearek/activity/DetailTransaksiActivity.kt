...
class DetailTransaksiActivity : AppCompatActivity() {
    ...
    private fun notif(){
        val title = "Bukti pembayaran berhasil diunggah"
            ...
            override fun onResponse(call: Call<ResponModel>, response: Response<ResponModel>) {
                ...
                if(respon != null){
                    if (respon.success == 1){
                        ...
                        notif()
                        Toast.makeText(this@DetailTransaksiActivity,"Berhasil menggunggah bukti pembayaran", Toast.LENGTH_SHORT).show()
                        onBackPressed()
                    }else{
                        loading_det_trans.visibility = View.GONE
    ...
    fun batalMobil(){
        ...
        ApiConfig.instanceRetrofit.updatemobil(id,0).enqueue(object : Callback<ResponModel> {
            override fun onResponse(call: Call<ResponModel>, response: Response<ResponModel>) {
                ...
                if (respon != null) {
                    if (respon.success == 1){
                        Toast.makeText(this@DetailTransaksiActivity, "Berhasil membatalkan transaksi", Toast.LENGTH_SHORT).show()
